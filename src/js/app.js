...
const CONFIG = Object.freeze({
  ...
  CROPS: {
    ...
    tomato: {
      name: "Tomate Indeterminado (Cultivo Secundario)",
      scientificName: "Solanum lycopersicum",
      ecThreshold: 2.5,
      slopePercentPerDs: 9.9
    },
    cucumber: {
      name: "Pepino (Cultivo Terciario)",
...
function initApp() {
  ...
  try { initCockpitScrollAndToc(); } catch (e) { console.error(e); }
...
/**
 * Inicializa el observador de scroll continuo para el TOC sticky y lazy-loading 3D
 */
function initCockpitScrollAndToc() {
  if (!document.body || !document.body.classList.contains("cockpit-page")) return;

  const tocItems = document.querySelectorAll(".cockpit-toc-item");
  const moduleSections = document.querySelectorAll(".cockpit-module-section");

  if (moduleSections.length > 0 && "IntersectionObserver" in window) {
    const observer = new IntersectionObserver((entries) => {
      entries.forEach(entry => {
        if (entry.isIntersecting) {
          ...
          // Inicialización perezosa de visores 3D al llegar por scroll
          if (id === "tab-estructura") {
            if (window.greenhouse3dViewer) {
              window.greenhouse3dViewer.onResize();
            } else {
              init3DViewerInstance();
            }
          } else if (id === "tab-pozo-artesanal") {
            if (window.pozo3dViewer) {
              window.pozo3dViewer.onResize();
            } else {
              initWell3DViewerInstance();
            }
          }
        }
      });
    ...
    moduleSections.forEach(sec => observer.observe(sec));
  }

  // Soporte de navegación por hash al cargar la página (ej. #tab-estructura, #tab-pozo-artesanal)
  if (window.location.hash) {
    ...
    if (targetEl) {
      setTimeout(() => {
        ...
        if (hash === "tab-estructura") init3DViewerInstance();
        if (hash === "tab-pozo-artesanal") initWell3DViewerInstance();
      }, 350);
    }
  }

  // Smooth scroll para clicks en TOC
  tocItems.forEach(item => {
    item.addEventListener("click", (e) => {
      ...
      if (targetEl) {
        ...
        item.classList.add("active");
        if (targetId === "tab-estructura") init3DViewerInstance();
        if (targetId === "tab-pozo-artesanal") initWell3DViewerInstance();
      }
    });
  });
}
...
function ensureThreeJsLoaded() {
  ...
  threeJsLoadingPromise = new Promise((resolve) => {
    ...
    loadScript("lib/three.min.js")
      ...
      .then(() => loadScript("well-3d.js"))
      .then(() => resolve());
  });
  return threeJsLoadingPromise;
}

function initWell3DViewerInstance() {
  const canvasBox = document.getElementById("pozo-3d-app-viewport");
  if (!canvasBox) return;

  ensureThreeJsLoaded().then(() => {
    if (window.Well3DViewer && !window.pozo3dViewer) {
      try {
        window.pozo3dViewer = new window.Well3DViewer("pozo-3d-app-viewport", {
          autoRotate: true,
          showCutaway: true
        });
        window._well3d = window.pozo3dViewer;
      } catch (err) {
        console.warn("[Pozo 3D Viewer] Error inicializando Well3DViewer:", err);
      }
    }
  });
}
  ...
  ensureThreeJsLoaded().then(() => {
    if (window.Greenhouse3DViewer && !window.greenhouse3dViewer) {
      try {
        ...
        window._gh3d = window.greenhouse3dViewer;
      } catch (err) {
...
function activateTab(targetTabId, scrollTargetId = null) {
  // Manejo de Layout Continuo en Cockpit
  if (document.body && document.body.classList.contains("cockpit-page")) {
    const targetElement = scrollTargetId ? document.getElementById(scrollTargetId) : document.getElementById(targetTabId);
    if (targetElement) {
      targetElement.scrollIntoView({ behavior: "smooth", block: "start" });
    }
    // Sincronizar TOC
    document.querySelectorAll(".cockpit-toc-item").forEach(item => {
    ...
    // Disparar visores 3D si se accede al módulo
    ...
    } else if (targetTabId === "tab-pozo-artesanal") {
      if (window.pozo3dViewer) {
        setTimeout(() => window.pozo3dViewer.onResize(), 60);
      } else {
        initWell3DViewerInstance();
      }
    }
  ...
  if ('serviceWorker' in navigator) {
    navigator.serviceWorker.register('./sw.js').then(() => {
      ...
      if (badge) {
        ...
        badge.innerHTML = '<span class="material-symbols-outlined" style="font-size:15px; vertical-align:middle; margin-right:4px;">offline_pin</span> Offline Listo (PWA)';
      }
    }).catch(() => {
      const badge = document.getElementById('pwa-status-badge');
...
const RealTimeWeatherService = {
  ...
  WMO_CODES: {
    0: { label: "Cielo Despejado", icon: "wb_sunny", class: "clear", isRain: false },
    1: { label: "Principalmente Despejado", icon: "partly_cloudy_day", class: "mostly-clear", isRain: false },
    2: { label: "Parcialmente Nublado", icon: "partly_cloudy_day", class: "partly-cloudy", isRain: false },
    3: { label: "Nublado / Convectivo", icon: "cloud", class: "cloudy", isRain: false },
    45: { label: "Neblina / Bochorno", icon: "foggy", class: "fog", isRain: false },
    48: { label: "Niebla con Escarcha", icon: "foggy", class: "fog", isRain: false },
    51: { label: "Llovizna Ligera", icon: "rainy", class: "drizzle", isRain: true },
    53: { label: "Llovizna Moderada", icon: "rainy", class: "drizzle", isRain: true },
    55: { label: "Llovizna Densa", icon: "rainy", class: "drizzle", isRain: true },
    61: { label: "Lluvia Ligera", icon: "rainy", class: "rain", isRain: true },
    63: { label: "Lluvia Moderada", icon: "rainy", class: "rain", isRain: true },
    65: { label: "Lluvia Fuerte Continua", icon: "rainy", class: "heavy-rain", isRain: true },
    80: { label: "Chubascos Dispersos", icon: "rainy", class: "showers", isRain: true },
    81: { label: "Chubascos Moderados", icon: "rainy", class: "showers", isRain: true },
    82: { label: "Chubascos Violentos", icon: "thunderstorm", class: "heavy-showers", isRain: true },
    95: { label: "Tormenta Eléctrica", icon: "thunderstorm", class: "thunderstorm", isRain: true },
    96: { label: "Tormenta con Granizo", icon: "thunderstorm", class: "thunderstorm", isRain: true },
    99: { label: "Tormenta Severa con Granizo", icon: "thunderstorm", class: "severe-thunderstorm", isRain: true }
  },
  ...
  getWmoInfo(code) {
    return this.WMO_CODES[code] || { label: "Condición Variable", icon: "cloud", class: "cloudy", isRain: false };
  },
  ...
  updateUI(data, isLive, timestamp, isErrorOffline = false) {
    ...
    if (topbarText) {
      const rainIcon = cur.precipitation > 0 ? "rainy" : (wmo.icon || "wb_sunny");
      topbarText.innerHTML = `<span class="material-symbols-outlined" style="font-size:15px; vertical-align:middle;">${rainIcon}</span> ${cur.temperature_2m.toFixed(1)}°C | <span class="material-symbols-outlined" style="font-size:15px; vertical-align:middle;">air</span> ${cur.wind_speed_10m.toFixed(0)} km/h | <span class="material-symbols-outlined" style="font-size:15px; vertical-align:middle;">water_drop</span> ${pSum.toFixed(1)} mm`;
    }
    ...
    if (liveIcon) liveIcon.innerHTML = `<span class="material-symbols-outlined" style="font-size:32px;">${wmo.icon}</span>`;
    ...
    for (let i = 0; i < count; i++) {
      ...
      html += `
        <div class="${cardClass}" title="${wmo.label} a las ${hourPart} (${rainMm} mm)">
          ...
          <span class="f-icon"><span class="material-symbols-outlined" style="font-size:20px;">${wmo.icon}</span></span>
          <span class="f-temp">${temp.toFixed(1)}°</span>
          <span class="f-rain" title="Probabilidad de lluvia ${prob}% | ${rainMm} mm">
            ${prob > 0 ? `<span class="material-symbols-outlined" style="font-size:12px; vertical-align:middle;">water_drop</span> ${prob}%` : `0%`}
          </span>
          <span class="f-wind"><span class="material-symbols-outlined" style="font-size:12px; vertical-align:middle;">air</span> ${wind.toFixed(0)}k</span>
        </div>
      `;
    }
  ...
  applyLiveToMainDashboard(data) {
    ...
    if (alertBox) {
      if (cur.temperature_2m >= 31.5 || tMax >= 32.0) {
        ...
        alertBox.innerHTML = `
          <div class="alert-icon"><span class="material-symbols-outlined" style="font-size:24px;">local_fire_department</span></div>
          <div class="alert-content">
            <strong style="display:inline-flex; align-items:center; gap:4px;"><span class="material-symbols-outlined" style="font-size:16px; color:#ef4444;">error</span> ALERTA EN VIVO — Estrés Térmico en Quíbor (Actual ${cur.temperature_2m.toFixed(1)} °C | Máx prevista ${tMax.toFixed(1)} °C):</strong>
        ...
        alertBox.innerHTML = `
          <div class="alert-icon"><span class="material-symbols-outlined" style="font-size:24px;">rainy</span></div>
          <div class="alert-content">
            <strong style="display:inline-flex; align-items:center; gap:4px;"><span class="material-symbols-outlined" style="font-size:16px; color:#38bdf8;">rainy</span> ALERTA EN VIVO — Previsión de Lluvias & Humedad en Quíbor (${cur.precipitation > 0 ? `${cur.precipitation.toFixed(1)} mm/h en curso` : `${pSum.toFixed(1)} mm previstos hoy, ${pProb}% prob.`}):</strong>
        ...
        alertBox.innerHTML = `
          ...
          <div class="alert-content">
            <strong style="display:inline-flex; align-items:center; gap:4px;"><span class="material-symbols-outlined" style="font-size:16px; color:#f59e0b;">air</span> ALERTA EN VIVO — Ráfagas Eólicas Intensas en Quíbor (${cur.wind_speed_10m.toFixed(1)} km/h, ráfagas de ${cur.wind_gusts_10m.toFixed(1)} km/h desde ${cardinal}):</strong>
  ...
  setOfflineFallbackUI() {
    ...
    const topbarText = document.getElementById("topbar-live-text");
    if (topbarText) topbarText.innerHTML = '<span class="material-symbols-outlined" style="font-size:15px; vertical-align:middle;">satellite_alt</span> Quíbor (Sin Red)';
  },
...
function calculateIrrigation() {
  ...
  if (elResEto) {
    ...
    elResEto.innerHTML = `${et0.toFixed(2)} mm/día ${isLive ? '<span class="tag" style="background:rgba(132,204,22,0.15); color:var(--apple-green-light); font-size:0.68rem; margin-left:0.3rem; display:inline-flex; align-items:center; gap:3px;"><span class="material-symbols-outlined" style="font-size:12px;">sensors</span> En Vivo</span>' : ''}`;
  }
  ...
  if (adviceText) {
    if (isPepper) {
      ...
      if (ecW > 2.5) {
        adviceText.innerHTML = `<strong style="display:inline-flex; align-items:center; gap:4px; color:#f59e0b;"><span class="material-symbols-outlined" style="font-size:16px;">warning</span> ALERTA TOMATE:</strong> Con agua de ${ecW} dS/m se excede el umbral de 2.5 dS/m. Requiere un <strong>${(lf * 100).toFixed(1)}% de lavado</strong> para evitar pudrición apical (*blossom-end rot*) por interferencia de calcio.`;
      } else {
...
function render12MonthFAO(density, surfaceM2, kc, hasMulch, lf, stageData) {
  ...
  months.forEach((m) => {
    ...
    if (tbody) {
      ...
      tr.innerHTML = `
        <td><strong>${m.name}</strong> ${m.peak ? '<span class="material-symbols-outlined" style="font-size:14px; vertical-align:middle; color:#ef4444;" title="Pico Cálido">local_fire_department</span>' : (m.min ? '<span class="material-symbols-outlined" style="font-size:14px; vertical-align:middle; color:#38bdf8;" title="Mínimo Anual">ac_unit</span>' : '')}</td>
...
function updateProductionStage() {
  ...
  if (FarmState.fertilizerRegime === "granulado") {
    ...
    if (fertAlertBox && fertAlertText) {
      ...
      if (fertAlertIcon) fertAlertIcon.innerHTML = '<span class="material-symbols-outlined" style="font-size:20px;">warning</span>';
    ...
    if (fertAlertBox && fertAlertText) {
      ...
      if (fertAlertIcon) fertAlertIcon.innerHTML = '<span class="material-symbols-outlined" style="font-size:20px;">lightbulb</span>';
    ...
    if (fertAlertBox && fertAlertText) {
      fertAlertBox.className = "alert-box alert-success";
      if (fertAlertIcon) fertAlertIcon.innerHTML = '<span class="material-symbols-outlined" style="font-size:20px;">star</span>';
...
function updateSprayPlanBanner() {
  ...
  if (currentSprayPlan === "biologico") {
    ...
    banner.innerHTML = `
      <div style="flex:1; min-width:280px;">
        <div style="display:flex; align-items:center; gap:0.6rem; flex-wrap:wrap; margin-bottom:0.3rem;">
          <span class="tag" style="background:rgba(16,185,129,0.25); color:#34d399; font-weight:800; font-size:0.75rem; display:inline-flex; align-items:center; gap:4px;"><span class="material-symbols-outlined" style="font-size:15px;">eco</span> PLAN PREVENTIVO RECOMENDADO (Base IPM)</span>
        ...
        <div style="display:flex; gap:0.4rem; flex-wrap:wrap; font-size:0.72rem;">
          <span class="tag" style="background:rgba(16,185,129,0.15); color:#34d399; border:1px solid rgba(16,185,129,0.3); display:inline-flex; align-items:center; gap:4px;"><span class="material-symbols-outlined" style="font-size:14px;">schedule</span> Carencia Promedio: 0 Días</span>
          <span class="tag" style="background:rgba(56,189,248,0.15); color:#38bdf8; border:1px solid rgba(56,189,248,0.3); display:inline-flex; align-items:center; gap:4px;"><span class="material-symbols-outlined" style="font-size:14px;">pest_control</span> Inocuo para Polinizadores</span>
          <span class="tag" style="background:rgba(168,85,247,0.15); color:#c084fc; border:1px solid rgba(168,85,247,0.3); display:inline-flex; align-items:center; gap:4px;"><span class="material-symbols-outlined" style="font-size:14px;">shield</span> Cero Riesgo de Resistencia</span>
        </div>
      </div>
    `;
    ...
    banner.innerHTML = `
      <div style="flex:1; min-width:280px;">
        <div style="display:flex; align-items:center; gap:0.6rem; flex-wrap:wrap; margin-bottom:0.3rem;">
          <span class="tag" style="background:rgba(245,158,11,0.25); color:#fbbf24; font-weight:800; font-size:0.75rem; display:inline-flex; align-items:center; gap:4px;"><span class="material-symbols-outlined" style="font-size:15px;">bolt</span> PROTOCOLO DE RESCATE QUÍMICO (IRAC / FRAC)</span>
        ...
        <div style="display:flex; gap:0.4rem; flex-wrap:wrap; font-size:0.72rem;">
          <span class="tag" style="background:rgba(244,63,94,0.15); color:#f43f5e; border:1px solid rgba(244,63,94,0.3); display:inline-flex; align-items:center; gap:4px;"><span class="material-symbols-outlined" style="font-size:14px;">warning</span> Respetar Carencia (1 a 7 días)</span>
          <span class="tag" style="background:rgba(245,158,11,0.15); color:#f59e0b; border:1px solid rgba(245,158,11,0.3); display:inline-flex; align-items:center; gap:4px;"><span class="material-symbols-outlined" style="font-size:14px;">sync</span> Rotar 7 Familias Químicas</span>
          <span class="tag" style="background:rgba(239,68,68,0.15); color:#f87171; border:1px solid rgba(239,68,68,0.3); display:inline-flex; align-items:center; gap:4px;"><span class="material-symbols-outlined" style="font-size:14px;">health_and_safety</span> Equipo de Protección Obligatorio</span>
        </div>
      </div>
    `;
  } else {
    ...
    banner.innerHTML = `
      <div style="flex:1; min-width:280px;">
        <div style="display:flex; align-items:center; gap:0.6rem; flex-wrap:wrap; margin-bottom:0.3rem;">
          <span class="tag" style="background:rgba(56,189,248,0.25); color:#38bdf8; font-weight:800; font-size:0.75rem; display:inline-flex; align-items:center; gap:4px;"><span class="material-symbols-outlined" style="font-size:15px;">sync</span> MANEJO INTEGRADO DE PLAGAS (MIP Completo)</span>
        ...
        <div style="display:flex; gap:0.4rem; flex-wrap:wrap; font-size:0.72rem;">
          <span class="tag" style="background:rgba(16,185,129,0.15); color:#34d399; border:1px solid rgba(16,185,129,0.3); display:inline-flex; align-items:center; gap:4px;"><span class="material-symbols-outlined" style="font-size:14px;">eco</span> 10 Herramientas Biológicas</span>
          <span class="tag" style="background:rgba(245,158,11,0.15); color:#f59e0b; border:1px solid rgba(245,158,11,0.3); display:inline-flex; align-items:center; gap:4px;"><span class="material-symbols-outlined" style="font-size:14px;">bolt</span> 11 Rescates Químicos</span>
          <span class="tag" style="background:rgba(56,189,248,0.15); color:#38bdf8; border:1px solid rgba(56,189,248,0.3); display:inline-flex; align-items:center; gap:4px;"><span class="material-symbols-outlined" style="font-size:14px;">shield</span> Bioseguridad ToBRFV</span>
        </div>
      </div>
    `;
  }
...
function calculateWellValidation() {
  ...
  if (boxVerdict && iconVerdict && titleVerdict && descVerdict) {
    if (flowLs >= 2.0 && autonomyDays >= 3.0) {
      ...
      iconVerdict.innerHTML = '<span class="material-symbols-outlined" style="font-size:24px; color:#10b981;">check_circle</span>';
      ...
      iconVerdict.innerHTML = '<span class="material-symbols-outlined" style="font-size:24px; color:#f59e0b;">warning</span>';
      ...
      iconVerdict.innerHTML = '<span class="material-symbols-outlined" style="font-size:24px; color:#ef4444;">cancel</span>';
...
function initWellDrillingCalculator() {
  ...
  function recalculateDrillMetrics() {
    ...
    if (elInterfStatus && elInterfStatusSub) {
      if (distanceM < 120) {
        elInterfStatus.innerHTML = '<span class="material-symbols-outlined" style="font-size:15px; vertical-align:middle; margin-right:3px;">warning</span> Alerta Crítica (Solapamiento)';
        ...
        elInterfStatus.innerHTML = '<span class="material-symbols-outlined" style="font-size:15px; vertical-align:middle; margin-right:3px;">warning</span> Interferencia Moderada';
        elInterfStatus.style.color = "#f59e0b";
        elInterfStatusSub.textContent = "Conos se intersectan. Se recomienda alternar bombeos.";
      } else {
        elInterfStatus.innerHTML = '<span class="material-symbols-outlined" style="font-size:15px; vertical-align:middle; margin-right:3px;">check_circle</span> Distancia Segura';
    ...
    if (tbodyBudget) {
      ...
      if (locKey === "ARTESANAL") {
        if (budgetTitle) budgetTitle.innerHTML = '<span class="material-symbols-outlined" style="font-size:17px; color:var(--emerald-light); vertical-align:middle; margin-right:4px;">assignment</span> Presupuesto Itemizado: Pozo Artesanal a Pico Ø 80 cm (+10 m con Bomba Industrial 5.5 HP)';
        ...
        if (budgetTitle) budgetTitle.innerHTML = `<span class="material-symbols-outlined" style="font-size:17px; color:var(--emerald-light); vertical-align:middle; margin-right:4px;">assignment</span> Presupuesto de Obra: ${locData.name}`;
...
// ==========================================================================
// SIMULADOR DINÁMICO DE RIEGO — TAB POZO ARTESANAL 60 M
// ==========================================================================
function initPozoArtesanalSimulator() {
  const sliderArea = document.getElementById('pozo-slider-area');
  const lblArea = document.getElementById('pozo-lbl-area');
  const selectCrop = document.getElementById('pozo-select-crop');
  const selectStage = document.getElementById('pozo-select-stage');

  const outDailyDemand = document.getElementById('pozo-calc-daily-demand');
  const outPumpTime = document.getElementById('pozo-calc-pump-time');
  const outWellBuffer = document.getElementById('pozo-calc-well-buffer');
  const outUtilization = document.getElementById('pozo-calc-utilization');
  const outSummaryTime = document.getElementById('pozo-calc-summary-time');

  if (!sliderArea || !selectCrop || !selectStage) return;

  const WELL_RECHARGE_LPS = 2.0; // 2.0 L/s = 7,200 L/h
  const PUMP_FLOW_LPH = 7200; // 2 HP sumergible a 60m ~ 7.2 m3/h
  const WELL_DIAMETER = 1.0;
  const WATER_COLUMN_DEPTH = 10.5; // 60m - 49.5m = 10.5m
  const STATIC_STORAGE_L = Math.PI * Math.pow(WELL_DIAMETER / 2, 2) * WATER_COLUMN_DEPTH * 1000; // ~8,246 Litros

  function updatePozoSimulation() {
    const areaM2 = parseFloat(sliderArea.value);
    if (lblArea) lblArea.textContent = `${areaM2.toLocaleString('es-ES')} m²`;

    const crop = selectCrop.value;
    const stage = selectStage.value;

    let kc = 1.05;
    if (stage === 'trasplante') kc = 0.60;
    else if (stage === 'vegetativo') kc = 0.80;
    else if (stage === 'floracion') kc = 1.00;
    else if (stage === 'pico') kc = 1.15;

    const etoBase = 5.8;
    let leachingFactor = 1.23; // 23% lixiviación FAO-56 para Quíbor

    const dailyDemandM3 = (areaM2 * (etoBase * kc * leachingFactor)) / 1000;
    const dailyDemandL = dailyDemandM3 * 1000;

    const pumpHours = dailyDemandL / PUMP_FLOW_LPH;
    const pumpMinutes = Math.round(pumpHours * 60);

    const maxDailyRechargeL = WELL_RECHARGE_LPS * 3600 * 24; // 172,800 L/día
    const utilizationPct = ((dailyDemandL / maxDailyRechargeL) * 100).toFixed(1);

    if (outDailyDemand) outDailyDemand.textContent = `${Math.round(dailyDemandL).toLocaleString('es-ES')} L`;
    if (outPumpTime) {
      if (pumpMinutes < 60) {
        outPumpTime.textContent = `${pumpMinutes} min`;
      } else {
        const h = Math.floor(pumpMinutes / 60);
        const m = pumpMinutes % 60;
        outPumpTime.textContent = `${h}h ${m}m`;
      }
    }
    if (outWellBuffer) outWellBuffer.textContent = `${Math.round(STATIC_STORAGE_L).toLocaleString('es-ES')} L`;
    if (outUtilization) outUtilization.textContent = `${utilizationPct} %`;
    if (outSummaryTime) {
      if (pumpMinutes < 60) {
        outSummaryTime.textContent = `${pumpMinutes} minutos al día`;
      } else {
        outSummaryTime.textContent = `${(pumpMinutes / 60).toFixed(1)} horas al día`;
      }
    }
  }

  sliderArea.addEventListener('input', updatePozoSimulation);
  selectCrop.addEventListener('change', updatePozoSimulation);
  selectStage.addEventListener('change', updatePozoSimulation);
  updatePozoSimulation();
}

if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', initPozoArtesanalSimulator);
} else {
  initPozoArtesanalSimulator();
}
