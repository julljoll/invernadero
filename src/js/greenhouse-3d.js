...
class Greenhouse3DViewer {
  constructor(containerId, options = {}) {
    ...
    this.currentCrop = 'pepper'; // 'pepper' (principal) | 'tomato' (secundario) | 'cucumber' (terciario)
  ...
  setCropVisualization(cropType) {
    if (!['pepper', 'tomato', 'cucumber'].includes(cropType)) return;
    this.currentCrop = cropType;
    this.buildCropVisualization(cropType);
  }

  buildCrop() {
    this.buildCropVisualization(this.currentCrop || 'pepper');
  }

  // 5. Cultivo Vegetal 3D Paramétrico (Pimentón, Tomate, Pepino)
  buildCropVisualization(cropType = 'pepper') {
    // Limpiar geometrías previas de la capa de cultivo
    while (this.layers.crop.children.length > 0) {
      const obj = this.layers.crop.children[0];
      this.layers.crop.remove(obj);
    ...
    const L = this.options.length;
    // 10 Camellones dobles a lo largo de los 50 m
    const rowX = [-8.8, -7.2, -4.8, -3.2, -0.8, 0.8, 3.2, 4.8, 7.2, 8.8];

    // Parámetros morfológicos según especie agronómica
    let cropMaxH = 1.1;  // Pimentón: porte bajo arbustivo 0.8 - 1.2 m
    let trellisH = 1.2;  // Altura tutorado pimentón
    let foliageColor = 0x15803d;
    let foliageRoughness = 0.82;

    if (cropType === 'tomato') {
      cropMaxH = 2.2;   // Tomate: hilo alto 2.2 m
      trellisH = 2.2;
      foliageColor = 0x166534;
    } else if (cropType === 'cucumber') {
      cropMaxH = 1.9;   // Pepino: espaldera 1.8 - 2.0 m
      trellisH = 2.0;
      foliageColor = 0x22c55e;
    }

    // Alambres horizontales de tutorado en cumbrera de camellón
    ...
    const plantBushGeo = new THREE.SphereGeometry(
      cropType === 'cucumber' ? 0.38 : (cropType === 'pepper' ? 0.32 : 0.35),
      7,
      7
    );

    // Frutos y materiales según cultivo
    let fruitGeo;
    let fruitMatRed, fruitMatGreen, fruitMatCucumber;

    if (cropType === 'pepper') {
      // Pimentón: cilindro cónico bloque 4 puntas
      fruitGeo = new THREE.CylinderGeometry(0.065, 0.045, 0.14, 8);
      fruitMatRed = new THREE.MeshStandardMaterial({ color: 0xb91c1c, roughness: 0.3, metalness: 0.08 });
      fruitMatGreen = new THREE.MeshStandardMaterial({ color: 0x15803d, roughness: 0.35, metalness: 0.08 });
    } else if (cropType === 'tomato') {
      // Tomate: esferas en racimos
      fruitGeo = new THREE.SphereGeometry(0.08, 7, 7);
      fruitMatRed = new THREE.MeshStandardMaterial({ color: 0xdc2626, roughness: 0.3, metalness: 0.1 });
      fruitMatGreen = new THREE.MeshStandardMaterial({ color: 0x65a30d, roughness: 0.4, metalness: 0.05 });
    } else if (cropType === 'cucumber') {
      // Pepino: cilindro alargado verde oscuro brillante
      fruitGeo = new THREE.CylinderGeometry(0.04, 0.038, 0.28, 8);
      fruitMatCucumber = new THREE.MeshStandardMaterial({ color: 0x065f46, roughness: 0.32, metalness: 0.1 });
    }

    // Sembrado de plantas en los camellones
    for (let r = 0; r < rowX.length; r++) {
      const x = rowX[r];
      const zStep = cropType === 'cucumber' ? 2.8 : 2.5;

      for (let z = -L / 2 + 1.5; z < L / 2; z += zStep) {
        const hStep = cropType === 'pepper' ? 0.35 : 0.55;
        const startH = 0.4;

        for (let h = startH; h <= cropMaxH; h += hStep) {
          const bush = new THREE.Mesh(plantBushGeo, foliageMat);
          bush.position.set(x + (Math.random() - 0.5) * 0.14, h, z + (Math.random() - 0.5) * 0.18);
          if (cropType === 'pepper') {
            bush.scale.set(1.15, 0.88, 1.15); // Arbusto denso y achaparrado
          } else if (cropType === 'cucumber') {
            bush.scale.set(1.1, 1.05, 0.8);  // Hojas grandes palmeadas
          } else {
            bush.scale.set(1.0, 1.2, 1.0);   // Enredadera vertical tomate
          }
          bush.castShadow = true;
          this.layers.crop.add(bush);
        }

        // Distribución de frutos colgantes reales
        if (cropType === 'pepper') {
          if (Math.random() > 0.25) {
            const isRed = Math.random() > 0.45;
            const pepper = new THREE.Mesh(fruitGeo, isRed ? fruitMatRed : fruitMatGreen);
            pepper.position.set(
              x + (Math.random() > 0.5 ? 0.18 : -0.18),
              0.55 + Math.random() * 0.35,
              z + (Math.random() - 0.5) * 0.2
            );
            pepper.rotation.z = (Math.random() - 0.5) * 0.2;
            this.layers.crop.add(pepper);
          }
        } else if (cropType === 'tomato') {
          if (Math.random() > 0.3) {
            const isRipe = Math.random() > 0.35;
            const tomato = new THREE.Mesh(fruitGeo, isRipe ? fruitMatRed : fruitMatGreen);
            tomato.position.set(x + 0.22, 1.0 + Math.random() * 0.6, z);
            this.layers.crop.add(tomato);
          }
        } else if (cropType === 'cucumber') {
          if (Math.random() > 0.25) {
            const cucumber = new THREE.Mesh(fruitGeo, fruitMatCucumber);
            cucumber.position.set(
              x + (Math.random() > 0.5 ? 0.2 : -0.2),
              0.7 + Math.random() * 0.5,
              z
            );
            cucumber.rotation.z = (Math.random() - 0.5) * 0.15;
            this.layers.crop.add(cucumber);
          }
        }
      }
    }
